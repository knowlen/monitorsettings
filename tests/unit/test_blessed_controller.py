...
class TestBlessedBacklightController:
    ...
    def test_initialization_failure(self, controller, mocker):
        ...
        mocker.patch.object(controller, "initialize", return_value=False)
        ...
        for call in mock_print.call_args_list:
            if call.args and (
                "No DDC/CI capable displays detected" in str(call.args[0])
                or "RED:Error" in str(call.args[0])
            ):
                error_printed = True
                break
